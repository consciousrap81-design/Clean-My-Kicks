...
const Email = ({ customerName, productName, productSize, carrier, trackingNumber, trackingUrl, orderUrl }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Your sneakers are on the way{trackingNumber ? ` — ${trackingNumber}` : ''}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={brand}><Text style={brandText}>CLEAN MY KICKS</Text></Section>

        <Heading style={h1}>They&rsquo;re on the way{customerName ? `, ${customerName}` : ''}!</Heading>
        <Text style={text}>
          Good news — your order just shipped. Use the tracking info below to
        ...
        {orderUrl && (
          <Text style={smallLink}>
            Or view your order details anytime: <a href={orderUrl} style={inlineLink}>{orderUrl}</a>
          </Text>
        )}
...
export const template = {
  component: Email,
  subject: (d: Props) => `Shipped — ${d.productName || 'your sneakers'} is on the way`,
  displayName: 'Shop Order Shipped',
  previewData: {
    customerName: 'Jordan',
