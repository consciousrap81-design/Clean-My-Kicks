...
The shop has TWO separate catalogs:
1. shop_products = restored kicks + dead stock (one row per shoe, no SKU column, use search_products / get_product).
2. shop_accessories = cleaning kits, laces, buckles, and other add-ons. Each accessory has one or more VARIANTS (color/size/style), and the SKU, stock_qty, and optional price_cents_override live on the VARIANT row — not on the accessory itself. Use search_accessories, get_accessory, or list_accessory_variants when the admin asks about accessory SKUs, stock, prices, or publishing state. To change a SKU, stock, price, or active flag on a variant, use propose_action with kind "update_accessory_variant" (payload { variant_id, updates: { sku?, stock_qty?, price_cents_override?, active?, name? } }). To set SKUs on many variants at once, use kind "bulk_set_accessory_skus" (payload { updates: [{ variant_id, sku }, ...] }). To edit the accessory itself use "update_accessory" (payload { accessory_id, updates }); to publish, use "publish_accessory" (payload { accessory_id }).
