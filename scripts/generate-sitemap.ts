...
const BASE_URL = "https://cleanmykicks.com";
...
async function fetchProducts(): Promise<SitemapEntry[]> {
  ...
  try {
    const res = await fetch(
      `${url}/rest/v1/shop_products?select=id,updated_at&status=in.(available,reserved)`,
      { headers: { apikey: key, Authorization: `Bearer ${key}` } }
    );
    if (!res.ok) return [];
    const rows: { id: string; updated_at: string | null }[] = await res.json();
    return rows.map((r) => ({
      path: `/shop/${r.id}`,
